...
- **Upload** (`GET /upload_video.html`, **`POST /submit_video_upload`**): page HTML must live in `templates/` only (no root `*.html` for these pages) so Vercel does not serve them as static files. **POST to a `.html` URL often returns HTTP 405** on static/CDN layers.
